<?php
/**
 * VowLMS — Investor Leads API (create)
 * Upload to: /home/goalvxiw/api.goalvow.com/api/investor-leads-create.php
 * Route:     POST /investor-leads
...
$range   = trim($body['investment_range'] ?? '');
$type    = trim($body['investor_type']    ?? '');
...
try {
    $db = getDb();
    $id = 'inv-' . bin2hex(random_bytes(10));

    $stmt = $db->prepare(
        'INSERT INTO investor_leads (id, name, email, organisation, investment_range, investor_type, message)
         VALUES (?, ?, ?, ?, ?, ?, ?)'
    );
    $stmt->execute([$id, $name, $email, $org, $range, $type, $message]);
