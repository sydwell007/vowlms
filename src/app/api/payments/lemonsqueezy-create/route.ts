...
export async function POST(request: Request) {
  ...
  if (!isBridgeConfigured()) {
    return badRequest("Lemon Squeezy checkout requires a connected backend and is unavailable in this environment.");
  }

  const country = await getLearnerCountry();

  try {
    return created(await bridgePost("/payments/lemonsqueezy-create-checkout", { parentSlugs: payload.parentSlugs, country }));
  } catch (e) {
    if (e instanceof BridgeError && e.status === 401) return unauthorized();
    if (e instanceof BridgeError) return badRequest(e.message);
    return serverError("Failed to create Lemon Squeezy checkout");
  }
}
