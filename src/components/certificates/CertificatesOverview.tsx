    ...
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {certificates.map((certificate) => {
        ...
        return <article key={certificate.certificateId} className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_14px_38px_rgba(6,17,31,0.05)]">
          <div className="relative aspect-[1.414/1] bg-slate-50">{template ? <Image src={template} alt="" fill sizes="(min-width: 1024px) 30vw, (min-width: 640px) 45vw, 90vw" className="object-cover" /> : <div className="absolute inset-3 flex flex-col items-center justify-center border border-[#ca9a21] px-4 text-center outline outline-1 outline-offset-2 outline-[#ca9a21]/60"><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-[#142b52]">GoalVow Academy</p><p className="mt-3 text-sm font-semibold uppercase text-[#142b52]">{certificate.courseName}</p><p className="mt-2 text-[9px] font-semibold uppercase text-slate-700">Certificate of Completion</p><ShieldCheck className="mt-4 text-[#ca9a21]" size={24} /></div>}</div>
